...
// CODE HERE
const changeHelloBtnColors = () => {
    sayHelloButton.style.backgroundColor = `black`;
    sayHelloButton.style.color = `white`;
}

sayHelloButton.addEventListener(`mouseover`, changeHelloBtnColors);
...
// CODE HERE
const changeHelloBtnBack = () => {
    sayHelloButton.style.backgroundColor = `#EFEFEF`;
    sayHelloButton.style.color = `black`;
}

sayHelloButton.addEventListener(`mouseout`, changeHelloBtnBack);
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get(`http://localhost:3000/repeat/defeat`)
    .then(response => {
...
// CODE HERE
const getQuery = () => {
    axios.get(`http://localhost:3000/query-test/?oh_crud&what_is_this&will_it_work/`)
    .then(response => {
        console.log(response.data);
    })
    .catch(err => console.log(err));
...
/*
    ...
    The console logged "You sent an empty query!"
   
    2: Send more than 1 query on the URL -- what happened? 
    The console looged "You sent more than 1 query!" and then proceeded to display an array of the queries I sent. It seems to want an object but I don't know why it should if this works as is.
*/
...
// CODE HERE 
let addToCart = document.querySelector(`#add-food`);

const createFood = (event) => {
    event.preventDefault();
    let foodInput = ` ` + document.querySelector(`#list`).value;
    ...
    .then(response => {
        console.log(response.data);
        let newP = document.createElement(`p`);
        newP.textContent = `You have added ${foodInput} to your cart! You currently have ${response.data} in your cart`;
        document.body.appendChild(newP);
        setTimeout(() => {
            newP.remove();
        }, 4000)
...
addToCart.addEventListener(`click`, createFood);